...
subtype SimpleStr, as Str,
   ...
   message   {
      inflate_message 'Attribute value [_1] is not a simple string', $_},
...
subtype LoadableClass, as NonEmptySimpleStr,
   message   {
      inflate_message
         'String [_1] is not a loadable class: [_2]', $_, $loadable_class_error
      },
...
subtype PositiveInt, as Int,
   inline_as { $_[0]->parent->inline_check($_)." and $_ >= 0" },
   message   {
      inflate_message 'Attribute value [_1] is not a positive integer', $_ },
   where     { $_ >= 0 };

subtype PositiveNum, as Num,
   inline_as { $_[0]->parent->inline_check($_)." and $_ >= 0" },
   message   {
      inflate_message 'Attribute value [_1] is not a positive number', $_},
   where     { $_ >= 0 };
...
# Private functions
sub _constraint_for_loadable_class {
   my $class = shift;
...
sub _exception_message_for_object_reference  {
   ...
   return inflate_message 'String [_1] is not an object reference', $x;
}

sub _exception_message_for_req_factory {
   my $x = shift;

   return inflate_message
      'Object [_1] is missing the new_from_simple_request method', blessed $x
      if blessed $x;

   return _exception_message_for_object_reference($x);
}

sub _exception_message_for_tracer {
   my $x = shift;

   return inflate_message 'Object [_1] is missing a frames method', blessed $x
      if blessed $x;
